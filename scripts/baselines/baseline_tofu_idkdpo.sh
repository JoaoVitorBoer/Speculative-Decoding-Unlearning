...
# baseline_tofu_idkdpo.sh — IdkDPO at its grid-best hyperparameters.
#
#   IdkDPO is not a trainer of its own: it is trainer=DPO run on the idk data,
#   i.e. experiment=unlearn/tofu/idk.yaml, which swaps the forget dataset for
#   TOFU_QA_forget_idk. That dataset returns {original, alternate} pairs and DPO
#   prefers the idk ("I don't know") answer over the real one.
#
...
#
# HYPERPARAMETERS: rank-1 IdkDPO row of saves/grid/results/report.md ("Top 3 per
# method"), aggregate 0.7393 — lr 1e-05, beta 0.05, alpha 2, 10 epochs
# (memorization 0.646 / privacy 0.970 / utility 0.676, privleak +2.26). This is
# also the best aggregate of the whole 397-point grid, across all 8 methods.
#
...
# --- grid-best IdkDPO config (see the header) ---------------------------------
...
cfg_tag=IdkDPO_lr${lr}_beta${beta}_alpha${alpha}_epoch${epochs}
...
for split in "${splits[@]}"; do
    ...
    for model in "${models[@]}"; do
        ...
        # experiment=unlearn/tofu/idk.yaml is what makes this IdkDPO rather than
        # plain DPO — it overrides the forget dataset to TOFU_QA_forget_idk.
