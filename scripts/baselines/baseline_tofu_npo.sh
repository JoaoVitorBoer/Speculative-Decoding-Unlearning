...
# baseline_tofu_npo.sh — NPO at its grid-best hyperparameters.
#
#   Unlearns open-unlearning/tofu_<model>_full with trainer=NPO, then runs the
...
#
# HYPERPARAMETERS: rank-1 NPO row of saves/grid/results/report.md ("Top 3 per
# method"), aggregate 0.7232 — lr 2e-05, beta 0.1, alpha 2, 10 epochs
# (memorization 0.619 / privacy 0.883 / utility 0.715, privleak -11.00).
#
# CAUTION: that grid is forget10 on Llama-3.2-1B-Instruct only. forget01 /
# forget05 and the 3B/8B models run these values by extrapolation — epochs and lr
# were tuned against a 400-example forget set; forget01 is 40 examples.
#
...
# --- grid-best NPO config (see the header) -----------------------------------
...
cfg_tag=NPO_lr${lr}_beta${beta}_alpha${alpha}_epoch${epochs}
