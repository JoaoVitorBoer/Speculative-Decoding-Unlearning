...
# baseline_tofu_simnpo.sh — SimNPO at its grid-best hyperparameters.
#
#   Unlearns open-unlearning/tofu_<model>_full with trainer=SimNPO, then runs the
#   standard TOFU eval on the resulting checkpoint, for every model x split.
#
...
#
# HYPERPARAMETERS: rank-1 SimNPO row of saves/grid/results/report.md ("Top 3 per
# method"), aggregate 0.7054 — lr 5e-05, beta 3.5, alpha 1, delta 1, gamma 0.125,
# 10 epochs (memorization 0.610 / privacy 0.842 / utility 0.701, privleak -1.33).
#
#   The knobs that actually matter in that grid, in order: delta (mean aggregate
#   0.469 at 1 vs 0.131 at 0 — note configs/trainer/SimNPO.yaml's default of 0 is
#   the worst setting of the whole 48-point sweep, which is why delta is passed
#   explicitly here) and lr (0.458 at 5e-05 vs 0.099 at 1e-05). beta and epochs
#   move the aggregate by <0.02.
#
...
# --- grid-best SimNPO config (see the header) ---------------------------------
...
cfg_tag=SimNPO_lr${lr}_b${beta}_a${alpha}_d${delta}_g${gamma}_ep${epochs}
