...
# baseline_tofu_graddiff.sh — GradDiff at its grid-best hyperparameters.
#
#   Unlearns open-unlearning/tofu_<model>_full with trainer=GradDiff (gradient
#   ascent on the forget set + alpha-weighted NLL on the retain set), then runs
...
#
# HYPERPARAMETERS: rank-1 GradDiff row of saves/grid/results/report.md ("Top 3
# per method"), aggregate 0.6691 — lr 2e-05, alpha 5, 5 epochs (memorization
# 0.556 / privacy 0.909 / utility 0.631, privleak -2.87).
#
#   Note forget_quality 1.6e-10: this config is a strong aggregate but the KS
#   test still separates it cleanly from the retain model, and gibberish 0.675 is
#   the second-lowest of any method's best point — GradDiff degrades fluency.
#
...
# --- grid-best GradDiff config (see the header) -------------------------------
...
cfg_tag=GradDiff_lr${lr}_alpha${alpha}_epoch${epochs}
